...
class CubeVisualizer:
    ...
    def visualize(self, faces, edges, e_orientations, corners, c_orientations):
        ...
        a = s**2
        # ######## Corners ###################################################
        # Do Front->Down->Back->Up
        for c in range(8):
            row = c // 2 * n
            if c % 2 == 0:
                c_ori = ba2int(c_orientations[c*2:(c+1)*2])
                front, side, top = c_ori, (c_ori + 1) % 3, (c_ori + 2) % 3
                ...
                c_ori = ba2int(c_orientations[c * 2:(c + 1) * 2])
                front, top, side = c_ori, (c_ori + 1) % 3, (c_ori + 2) % 3
        ...
        for i, c in enumerate(corner_idxs):
            ...
            if i % 2 == 0:
                ...
                # top, side, front = c_ori % 3, (c_ori + 1) % 3, (c_ori + 2) % 3
                img[row, -1] = self.corner_c_map[corners[c], side]
                img[row+1, -1] = self.corner_c_map[corners[c], side]
                img[row, -2] = self.corner_c_map[corners[c], top]
                img[row+1, -2] = self.corner_c_map[corners[c], front]

        # Do Right (blue) face view
        corner_idxs = [1, 7, 3, 5]
        for i, c in enumerate(corner_idxs):
        ...
        for e in range(12):
            ...
            for i in range(s):
                ...
                if e % 3 == 0:
                    ...
                    img[row, 2+i] = self.edge_c_map[edges[e_idx], top]
                    img[row + 1, 2+i] = self.edge_c_map[edges[e_idx], front]
                elif e % 3 == 1:
                    front, left = e_ori, (e_ori + 1) % 2
                    img[row+2+i, 0] = self.edge_c_map[edges[e_idx], left]
        ...
        row = 12 // 3 * n
        for i in range(s):
            e_ori = int(e_orientations[0])
            front, top = e_ori, (e_ori + 1) % 2
            img[row, 2 + i] = self.edge_c_map[edges[0], top]
            img[row + 1, 2 + i] = self.edge_c_map[edges[0], front]
        ...
        for i in range(s):
            e_idx = e * s + i
            e_ori = int(e_orientations[e_idx])
            front, top = e_ori % 2, (e_ori + 1) % 2  # Orientation is inverted since edge is viewed upside down
            img[row+2+s, 2+i] = self.edge_c_map[edges[e_idx], top]
            img[row+2+s + 1, 2+i] = self.edge_c_map[edges[e_idx], front]
    ...
    def visualize_compact(self):
        img = np.zeros((self.n * 4, self.n * 5, 3))
